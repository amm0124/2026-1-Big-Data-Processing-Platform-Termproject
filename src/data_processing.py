...
def display_data_summary(path):
    ...
    print("Saving data summary to ../data/")
    # Save to files
    team_sheets_all.to_csv("../data/team_sheets.csv", index=False)
    all_events.to_csv("../data/all_events.csv", index=False)

    with open("../data/n_frames.txt", "w") as f:
        f.write(str(n_frames))

    print("Data summary saved to ../data/")
